...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    Concat, Conv, ConvTranspose, Detect, DWConv, DWConvTranspose2d, Ensemble, Focus,AKConv,
                                    GhostBottleneck, GhostConv, Segment, CoordAtt, Involution,space_to_depth,BasicRFB_a,SimAM,ResNet_RFB,ResNet_ACmix,ACmix,GAM_Attention,FasterNeXt,Bottleneck_GhostConv,C2f_GhostConv)
...
from ultralytics.nn.fb import FocalNextBlock
from ultralytics.nn.modules import RepVGGBlock, SimSPPF, SimConv, RepBlock, Transpose
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, CoordAtt,AKConv, 
                 GAM_Attention,Involution,BasicRFB_a,ResNet_ACmix,ACmix,BiLevelRoutingAttention, Attention, AttentionLePE,ResNet_RFB,BiFormerBlock,FocalNextBlock,RepVGGBlock, SimSPPF, SimConv, RepBlock, Transpose,FasterNeXt,Bottleneck_GhostConv,C2f_GhostConv):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x,RepVGGBlock,RepBlock,FasterNeXt):
