...
class ACmixblock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, k=1, s=1, p=None, g=1,
...
class ResNet_RFB(nn.Module):
    # ResNet bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super(ResNet_RFB, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        # self.cv2 = ACmixblock(c_, c_, kernel_att=7, head=4, kernel_conv=3, stride=1, dilation=1)
        self.cv2 = BasicRFB_a(c_, c_, stride=1, scale=0.1)
        self.cv3 = Conv(c_, c2, 1, 1)
        self.add = shortcut and c1 == c2
...
class Bottleneck_GhostConv(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
...
class C2f_GhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck_GhostConv(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
